...
function drawWeapon(){
    for(let  k=0 ; k<4 ; k++) {
        let weaponId = contDoublon();
        if ( k === 0){
            let weapon1 = new Weapon ("EPEE",20, weaponId)
            $('#'+ weaponId ).addClass('weapon1');
            weapons.push(weapon1);         
        }
        else if ( k === 1){
            let weapon2 = new Weapon ("LASER",35, weaponId)
            $('#'+ weaponId ).addClass('weapon2');
          weapons.push(weapon2);
        }
        else if ( k === 2){
            let weapon3 = new Weapon ("MAIN",15, weaponId)
            $('#'+ weaponId ).addClass('weapon3');
          weapons.push(weapon3);
        } 
        else if ( k === 3){
            let weapon4 = new Weapon ("PISTOLET",25, weaponId)
